...
class Result extends Component {
  ...
  render() {
    ...
    //var self = this;
    var labels = null === this.state.label_list ? <Loading /> : this.state.label_list.map((label, index) => {
      return (
    ...
    var phrases = null === this.state.phrase_list ? <Loading /> : 0 === this.state.phrase_list.length ? 'Phrases unavailable' : this.state.phrase_list.map((phrase, index) => {
    ...
    var entities = null === this.state.entity_list ? <Loading /> : 0 === this.state.entity_list.length ? 'Entities unavailable' : this.state.entity_list.map((entity, index) => {
